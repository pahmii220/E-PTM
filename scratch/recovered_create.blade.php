                    ...
                    <select name="deteksi_dini_id_select" id="deteksi_dini_id_select" class="form-select select2-deteksi" style="background-color: #f0fdfa;">
                        ...
                        @foreach ($daftarDeteksi as $d)
                            ...
                            <option value="{{ $d->id }}"
                                ...
                                {{ (isset($deteksiTerpilih) && $d->id == $deteksiTerpilih->id) ? 'selected' : '' }}>
        ...
        $(document).ready(function() {
            ...
            $('#deteksi_dini_id_select').on('change', function() {
                ...
                if (val) {
                    ...
                    const manualPTM = [];
                    const manualList = ['Jantung Koroner', 'Stroke', 'Asma Bronkial', 'PPOK (Paru Kronis)', 'Thalasemia', 'Kanker Payudara', 'Kanker Leher Rahim (Serviks)', 'Katarak', 'Glaukoma', 'Gangguan Pendengaran', 'Gangguan Penglihatan'];
                    diagnosaList.forEach(function(d) {
                        if (manualList.includes(d)) {
                            manualPTM.push(d);
                        }
                    });

                    // Logika Saran Tindak Lanjut
                    let saranArray = [];
                    activeRecomJenis = "edukasi";

                    if (hasHipertensi) {
                        saranArray.push("- Hipertensi: Berikan edukasi diet rendah garam (DASH), batasi asupan makanan olahan/asin, anjurkan olahraga aerobik sedang (seperti jalan cepat) selama 30 menit sehari, kelola stress, dan wajib kontrol tensi darah ke Puskesmas setiap 2 minggu sekali.");
                    }
                    if (hasDiabetes) {
                        saranArray.push("- Gula Darah Tinggi: Berikan konseling pembatasan konsumsi karbohidrat sederhana/manis, perbanyak konsumsi serat (sayur & buah), anjurkan jalan kaki/olahraga rutin, serta pantau kadar gula darah secara teratur.");
                    }
                    if (hasObesitas) {
                        saranArray.push("- Obesitas/Gizi: Rekomendasikan pola makan seimbang (Piring Makanku), kurangi makanan berlemak tinggi, gorengan, dan manis, serta sarankan peningkatan aktivitas fisik minimal 150 menit per minggu.");
                    }
                    if (manualPTM.length > 0) {
                        activeRecomJenis = "rujukan";
                        saranArray.push("- Temuan PTM (" + manualPTM.join(', ') + "): Rujuk pasien ke fasilitas kesehatan tingkat lanjut (Rumah Sakit) untuk konsultasi dokter spesialis dan penanganan medis lebih mendalam.");
                    }

                    if (saranArray.length === 0) {
                        activeRecomJenis = "anjuran_gaya_hidup";
                        saranArray.push("- Normal: Pertahankan gaya hidup sehat, konsumsi gizi seimbang, hindari rokok, cukup istirahat, dan lakukan skrining kesehatan rutin minimal 1 tahun sekali.");
                    }

                    activeRecomSaran = "Saran Rekomendasi Medis:\n" + saranArray.join("\n\n");
